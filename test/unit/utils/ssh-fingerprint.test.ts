...
describe('SSH Key Fingerprint utilities', () => {
  ...
  describe('getSSHKeyFingerprint', () => {
    it('should calculate correct fingerprint for RSA key', () => {
      const fingerprint = getSSHKeyFingerprint(sampleRSAKey);
    ...
    it('should calculate correct fingerprint for ED25519 key', () => {
      const fingerprint = getSSHKeyFingerprint(sampleED25519Key);
      expect(fingerprint).toMatch(/^SHA256:[A-Za-z0-9+/]+$/);
      expect(fingerprint.startsWith('SHA256:')).toBe(true);
      expect(fingerprint.length).toBeGreaterThanOrEqual(47);
    ...
    it('should handle keys with extra whitespace', () => {
      ...
      const fingerprint = getSSHKeyFingerprint(keyWithSpaces);
      const normalFingerprint = getSSHKeyFingerprint(sampleRSAKey);
      expect(fingerprint).toBe(normalFingerprint);
    });

    it('should throw error for invalid key format', () => {
      expect(() => getSSHKeyFingerprint('invalid-key')).toThrow('Invalid SSH public key format');
      expect(() => getSSHKeyFingerprint('ssh-rsa')).toThrow('Invalid SSH public key format');
      expect(() => getSSHKeyFingerprint('')).toThrow('Invalid SSH public key format');
    });
  });

  describe('getSSHKeyInfo', () => {
    it('should extract correct info from RSA key', () => {
      const info = getSSHKeyInfo(sampleRSAKey);
      expect(info.type).toBe('ssh-rsa');
      expect(info.fingerprint).toMatch(/^SHA256:[A-Za-z0-9+/]+$/);
    });

    it('should extract correct info from ED25519 key', () => {
      const info = getSSHKeyInfo(sampleED25519Key);
      expect(info.type).toBe('ssh-ed25519');
      expect(info.fingerprint).toMatch(/^SHA256:[A-Za-z0-9+/]+$/);
    });

    it('should handle keys without comment', () => {
      ...
      const info = getSSHKeyInfo(keyNoComment);
      expect(info.type).toBe('ssh-rsa');
      expect(info.fingerprint).toMatch(/^SHA256:[A-Za-z0-9+/]+$/);
    });

    it('should handle keys with multi-word comment', () => {
      ...
      const info = getSSHKeyInfo(keyWithComment);
      expect(info.type).toBe('ssh-rsa');
      expect(info.fingerprint).toMatch(/^SHA256:[A-Za-z0-9+/]+$/);
    });

    it('should throw error for invalid key format', () => {
      expect(() => getSSHKeyInfo('invalid-key')).toThrow('Invalid SSH public key format');
    });
  });
});